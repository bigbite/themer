      ...
      <div className="colorField">
        ...
        {paletteVisible && (
          <ColorPicker
            ...
            defaultValue="#ff9900"
            onChange={($val) => {
              props.setPaletteData(props.slug, $val);
              props.color = $val;
            }}
          />
        )}
        <TextControl label={__('Name')} value={props.name} onChange={(e) => console.log(e)} />
        <TextControl label={__('Slug')} value={props.slug} onChange={(e) => console.log(e)} />
      </div>
    </>
  );
};
